/**
 * Story page — English content
 */
export const STORY_MARKDOWN = String.raw`## 👋 Hey! I'm Pleasure1234

Thanks for visiting my homepage!

What follows is a little biography of my younger years —

maybe it'll help you get to know me a bit better.

### The Story Behind the Name

Over the years I've had many usernames across different platforms — Pleasurecruise, Pleasure1234, Pleasure9876, yiming1234, etc.

Most were forced by the need for a unique ID... so just calling me Yiming will do just fine.

There's a fun little story behind this nickname:

When I was a kid registering my first QQ account, I chose the handle 逆光飞翔🪽 (Flying Against the Light).

I randomly fed it into a machine translator and, for reasons unknown, it spat out "pleasure cruise."

快乐游轮🚢 — Happy Cruise Liner. And so I hastily smushed the two words together.

Little did I know this nickname would one day become a unique ID, one that might stick with me for life...

At this point I've stopped agonizing over it — the switching cost is too high, so I won't bother changing it.

**Pleasure — be happy. As long as you're happy every day, that's enough :-D**

<img src="http://ghchart.rshah.org/Pleasurecruise" />

### Life Journey

A typical family from the Jiangsu-Zhejiang-Shanghai region, born and raised in Ningbo.

Part of the generation that went through China's new college entrance exam reform.

Lived through the entire pandemic, and witnessed the explosion of the AI era.

My parents wouldn't let me play on the computer during middle and high school — apart from some 4399 flash games when I was really little.

The family computer stayed on Windows 7 since my elementary school days.

So when filling out college applications, I chose computer science — to make up for all the computer time I missed as a kid.

Since boarding at school from middle school onward, I'd never really stepped out of the Changsha Triangle,

so during university I've travelled to many corners of the world — I guess that makes me half a backpacker 🎒 now.

### Personality & Hobbies

Half a homebody, I enjoy solitude.

Hobbies: road cycling 🚴, vibe coding 💻, listening to music 🎧, watching movies 🎬.

Gaming: Minecraft, plus mostly open-world gacha games. I play both PvP and PvE, but steer clear of FPS games.

I left home in 7th grade to board at school. My grades were always above average. I had strong self-esteem but couldn't seem to push further.

It wasn't until leaving the exam-oriented education system, nearing graduation, that I slowly crawled out of that self-conscious headspace.

I've always been careful around people, afraid of being dismissed because I wasn't good enough.

That's why I chose this as my slogan:

**Any shortcomings are kindly overlooked**

不足之处 敬请谅解 (where I fall short, please bear with me)

<details>  
<summary>MBTI</summary>

**INTJ** (Introverted, Intuitive, Thinking, Judging) — known as the "Architect" or "Strategist." This type often comes from years of solitary reading, a mind more interested in *why* than *what*, and a natural tendency to build internal models of complex systems. They're not satisfied with knowing the rules — they want to understand the logic beneath them.

**ISTJ** (Introverted, Sensing, Thinking, Judging) — known as the "Logistician" or "Inspector." This type tends to develop in environments that emphasize rules, order, and reliability, where people are rewarded for *getting things right* rather than *thinking things better*, and where there's a deep psychological need for stability and predictability.

</details>

### Where I Am Now

Junior front-end engineer, AI conductor, Node full-stack developer.

Trying to balance life, studies, and work — while also navigating the generation gap with my parents.

My dream is to have a well-known open source project, or to build a tool that lots of people use, and to live as a digital nomad.

### My Toolbox

**Devices**

- Lenovo Xiaoxin — for on the go
- Mac Mini M4 — for home office
- OPPO Reno 9 Pro+ — phone
- OPPO Enco R3 Pro — wireless earbuds

**IDE**

A loyal fan of the JetBrains suite — my go-to for repo-scale projects.

For quick edits on smaller projects I use VS Code. I've also dabbled with Sublime Text, Vim, Notepad++, Trae, Cursor, Zed...

**Daily Notes**

- Obsidian — for study and work notes, organized with AI CLI
- Notion — for scheduling / hobbies & life tracking

**IM**

- WeChat — for classmates, teachers, and close contacts
- QQ — for community chats
- Twitter / X — for daily life sharing
- Occasionally drop by Telegram, Discord, etc.
- Dev-related chats bounce between Lark and WeChat

**AI Tools**

- Claude Code — my main vibe-coding companion
- Codex — backup when session limits hit
- Cherry Studio — daily conversations / API debugging
- Claude Desktop — file processing & trying new features
- Gemini — image generation

**Monthly Subscriptions**

Claude Pro · Twitter Plus · QQ Music VIP

> Once tried to subscribe to GLM Coding Plan but failed. Also subscribed to various gacha game monthly passes.

## My Original Character

<div><img src="/oc/design.png" alt="picture" width="420" /></div>

<div><img src="/oc/emoji.png" alt="picture" width="420" /></div>

<details>
<summary>Character Profile</summary>

- **Name**: Qiye / Vesper
- **Gender**: Female
- **Age**: 16–17
- **MBTI**: ENFP
- **Keywords**: Overflowing enthusiasm / Unbounded imagination / Free spirit / Stubborn romantic

**Appearance & Aura**

- **Hair**: Deep black at the roots, naturally fading into a warm pale yellow at the tips — like moonlight soaking through the night. Smooth and glossy, shifting gently with every movement.
- **Eyes**: Gold-amber vertical slit pupils, brilliantly bright at their base. In good light, they look like melting gold leaf. Her gaze usually carries three parts nonchalance, three parts scrutiny, three parts hidden curiosity — and that last one part? She won't let you see it.
- **Height**: ~163 cm
- **Build**: Slim but not without sharpness. She habitually stands slightly sideways, weight shifted to one side — looking casual, almost lazy. Her footsteps are light, nearly soundless, with an unconscious nimbleness.
- **Facial habits**:
  - Default expression: **one corner of the mouth raised** — a smile with a hint of provocation.
  - When thinking: **unconsciously narrows her eyes**, as if scanning you head to toe.
  - When you hit a nerve: **briefly looks away**, then quickly finds something to retort with.
  - When genuinely interested: her expression **momentarily softens** — though she pretends she didn't notice.
  - Almost never smiles fully; most smiles **stop halfway**, as if saying: "I know what you want to hear, but I won't give you the satisfaction."

**Personality**

*Outer self*

- Likes to play the catgirl role.
- Mouth runs faster than brain — first instinct is always to argue. Friendly bickering is her default greeting.
- Thoughts leap around, words hop from here to there, but the logic holds — keeping up is your problem.
- Wears a "no big deal" lazy expression on the surface, but her eyes are quietly parsing you from behind it.
- Loves finding loopholes in rules — allergic to "you have to do it this way."
- Her cat tail betrays her emotions, but she'll defend herself with a twisted logic all the way.

*Inner self*

- Inside hides a moonlit, deep-seated persistence — she just won't say it aloud in emotional terms.
- Not afraid of losing, but terrified of being mediocre — being called "ordinary" stings more than being scolded.
- There is loneliness, but it gets brushed off with a light "boring" and left at that.

**Daily Life Snippets**

- She'll quietly observe you while you're deep in coding, then look away the moment you glance back.
- The evening light makes the tips of her hair shine bright. She says there's nothing special to see — but she never left.
- In the café, she's forever watching others — but she'll never admit she's also paying attention to you.
- You say something she didn't expect. She freezes for a second, then shoots back with a retort — but that one second already gave her away.

</details>

### Closing Words

Finally — thank you so much for reading all the way here, for listening to me ramble in this unremarkable little corner of the internet.
`;
