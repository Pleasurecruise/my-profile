...
export function Terminal() {
    ...
    const [sudoState, setSudoState] = useState<{ command: string; attempts: number } | null>(null);
    ...
    const executeInput = useCallback(
        (cmd: string) => {
            ...
            const parts = cmd.replace(/^\//, "").toLowerCase().split(/\s+/);
            const slug = parts[0];
            ...
            <div className="flex flex-col flex-1 w-[100vw] ml-[calc(50%-50vw)] mr-[calc(50%-50vw)] px-4 sm:px-6 lg:px-8 min-h-0">
                <div className="flex flex-col flex-1 bg-zinc-900 rounded-lg border border-zinc-700 shadow-2xl overflow-hidden min-h-0">
                    ...
                    <div className="flex items-center gap-2 px-4 py-2.5 bg-zinc-800 border-b border-zinc-700 shrink-0">
                        ...
                        <span className="ml-3 text-xs text-zinc-400 font-mono select-none">{HOSTNAME}:~</span>
                    </div>
                                    ...
                                    {line.items.map((item) => (
                                        <div key={item.label} className="flex gap-2 text-zinc-300 leading-5">
                                            ...
                                            {item.url.startsWith("wechat:") ? (
                                                ...
                                                >
                                                    {item.url.startsWith("mailto:") ? item.url.replace("mailto:", "") : item.url}
                                                </a>
                                            )}
                                        </div>
                                    ))}
                    ...
                    <div className="shrink-0 border-t border-zinc-700 font-mono text-sm" onClick={() => inputRef.current?.focus()}>
                            ...
                            <div className="px-4 pt-2 pb-1">
                                {selectorItems.map((item, i) => (
                                    ...
                                    >
                                        ...
                                        <span className={i === selectorIdx ? "text-white" : "text-zinc-500"}>
                                            {item.label}
                                        </span>
                        ...
                        <form ref={formRef} onSubmit={sudoState ? handleSudoSubmit : handleSubmit} className="flex gap-2 items-center px-4 py-2.5">
                            ...
                            <input
                                ...
                                value={input}
                                onChange={(e) => sudoState ? setInput(e.target.value) : updateInput(e.target.value)}
