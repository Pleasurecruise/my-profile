...
// Icons with dark/black logos that need to be inverted in dark mode
...
function getAppIcon(appName: string): { url: string; invert: boolean } | null {
	...
	for (const [key, url] of Object.entries(APP_ICONS)) {
		if (lower.includes(key)) {
			return { url, invert: INVERT_IN_DARK.has(key) };
		}
	}
	return null;
}
...
export default function AmIOkPage() {
	...
	const fetchStatus = async () => {
		try {
			...
			const data = (await res.json()) as Status | null;
			setStatus(data);
		} catch {
			setStatus(null);
		} finally {
							...
							{apps.map((appName, appIndex) => {
								...
								return (
									<div key={appName} className="flex items-center gap-3">
										{appIndex > 0 && <div className="h-8 w-px bg-border" />}
										{icon ? (
											<img
												src={icon.url}
												alt={appName}
