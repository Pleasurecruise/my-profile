...
function parseIncompleteMarkdown(text: string): string {
  ...
  const linkPrefix = linkMatch?.[1];
  if (linkMatch && linkPrefix) {
    // If we have an unterminated [ or ![, remove it and everything after
    const startIndex = result.lastIndexOf(linkPrefix);
...
const hardenReactMarkdown: typeof hardenReactMarkdownModule = (() => {
  if (typeof hardenReactMarkdownModule === "function") {
    return hardenReactMarkdownModule;
  }

  const fallback = (hardenReactMarkdownModule as { default?: typeof hardenReactMarkdownModule })
    .default;
  ...
  throw new TypeError("harden-react-markdown did not export a callable function");
})();
