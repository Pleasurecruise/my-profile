...
function formatReadingTime(cjk: number, latin: number): string {
  const minutes = Math.ceil(cjk / CJK_READING_SPEED + latin / LATIN_READING_SPEED);
  return `${minutes} min`;
}
...
export function ArticleHeader({
  ...
  const readingTime = stats && total > 0 ? formatReadingTime(stats.cjk, stats.latin) : undefined;
