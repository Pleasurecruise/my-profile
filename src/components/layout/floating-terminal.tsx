...
const config: TerminalConfig = {
  ...
  profile: {
    ...
    founded: [{ label: "CompPsyUnion", url: "https://github.com/CompPsyUnion" }],
    contributions: [
      { label: "cherry-studio", url: "https://github.com/CherryHQ/cherry-studio" },
...
export function FloatingTerminal() {
  ...
  useEffect(() => {
    ...
    const onMove = (e: PointerEvent) => {
      ...
      if (ia.kind === "drag") {
        setPos({
          x: Math.max(0, Math.min(window.innerWidth - MIN_W, ia.startPos.x + dx)),
          y: Math.max(0, Math.min(window.innerHeight - MIN_H, ia.startPos.y + dy)),
        });
      } else {
      ...
      {open && !isMobile && (
        <div
          style={{
            ...
            height: size.h,
            zIndex: 50,
            transition: active ? "none" : "opacity 200ms ease-out, transform 200ms ease-out",
        ...
        >
          <div className="absolute inset-0 pointer-events-none" style={{ zIndex: 10 }}>
            {(["n", "s", "w", "e", "nw", "ne", "sw", "se"] as const).map((dir) => (
              <div
                key={dir}
                className={cn(
                  "absolute pointer-events-auto",
                  dir === "n" && "top-0 left-3 right-3 h-1.5 cursor-n-resize",
                  dir === "s" && "bottom-0 left-3 right-3 h-1.5 cursor-s-resize",
                  dir === "w" && "left-0 top-3 bottom-3 w-1.5 cursor-w-resize",
                  dir === "e" && "right-0 top-3 bottom-3 w-1.5 cursor-e-resize",
                  dir === "nw" && "top-0 left-0 w-3 h-3 cursor-nw-resize",
                  dir === "ne" && "top-0 right-0 w-3 h-3 cursor-ne-resize",
                  dir === "sw" && "bottom-0 left-0 w-3 h-3 cursor-sw-resize",
          ...
          <Terminal config={config} onClose={() => setOpen(false)} onDragStart={handleDragStart} />
        </div>
      )}
    </>
  );
